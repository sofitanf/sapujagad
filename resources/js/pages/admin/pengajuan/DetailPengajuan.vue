<template>
	...
	<div>
			...
			<div class="col-12 md:col-6">
				<table>
					...
					<tr>
						<td>Status</td>
						<td>:</td>
						<td v-if="data.status === 'Terkirim'">Pengajuan Baru</td>
		...
		<div v-if="admin" class="card grid p-fluid">
...
export default {
	...
	computed: {
		...
		admin() {
			return this.$store.getters?.user?.role === "Admin" ? false : true;
		},
	},
	methods: {
		...
		async editPengajuan() {
			if (
				...
				this.form.jadwal < this.data.created_at
			) {
				this.$toast.add({
					severity: "error",
					summary: "Gagal",
